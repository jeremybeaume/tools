...
foreach($srv in $svc_list) {
    if($(Test-Path "HKLM:\SYSTEM\CurrentControlSet\Services\$srv")) {
        if( $(Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\$srv").Start -eq 4) {
            Write-Host "        [i] Service $srv already disabled"
        } else {
            Write-Host "        [i] Disable service $srv (next reboot)"
            Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\$srv" -Name Start -Value 4
            $need_reboot = $true
        }
    } else {
        Write-Host "        [i] Service $srv already deleted"
    }
}
...
if($need_reboot) {
    ...
    if($MyInvocation.UnboundArguments.Contains("-Delete") -Or $MyInvocation.UnboundArguments.Contains("-delete")) {
        ...
        $svc_list = @("WdNisSvc", "WinDefend", "Sense")
        foreach($srv in $svc_list) {
            Delete-Show-Error "HKLM:\SYSTEM\CurrentControlSet\Services\$srv"
        }
